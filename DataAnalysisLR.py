...
def train_and_evaluate(X: pd.DataFrame, Y: pd.Series, k: int = 5):
    """
    使用线性回归模型进行 k 折交叉验证训练，并计算指标与特征系数。
    返回值：
        - eval_metrics: 字典，包含平均 R2、RMSE 和 MAE
        - feature_importances: DataFrame，使用系数绝对值表示
        - shap_means: DataFrame，占位（线性回归无 SHAP 支持）
        - train_rmse_folds: 空列表
        - val_rmse_folds: list，每个 fold 的 RMSE
...
def main():
    ...
    plot_true_vs_pred(all_true, all_pred, save_path='True_vs_Predicted_Values-LR.png')
